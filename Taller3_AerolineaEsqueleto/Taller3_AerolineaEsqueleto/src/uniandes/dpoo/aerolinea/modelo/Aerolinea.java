...
//no se porque me sale en rojito esto
//como qye hay que cambiar el path e importar json desde internet pero ni idea
...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        for (Avion avioncito:aviones){
        	String nombreAvioncito=avioncito.getNombre();
        	if (nombreAvioncito==nombreAvion) {
        		avionSeleccionado=avioncito;
        	}
        }
        ...
        Ruta estaRuta=this.getRuta(codigoRuta);	
        if (estaRuta==null) {
        	throw new Exception("La ruta no existe");
        }
        
        Vuelo nuevoVuelo= new Vuelo(estaRuta, fecha, avionSeleccionado);
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	int capacidadRestante = vuelo.getAvion().getCapacidad() - vuelo.getTiquetes().size();
        if (capacidadRestante < cantidad) {
            throw new VueloSobrevendidoException(vuelo);
        }
         // acá me faltaría obener la calculadora según tepmorada
        String[] mesesTemporadaAlta= new String [] {"enero", "febrero", "marzo", "abril", "mayo", "septiembre", "octubre", "noviembre"};
        CalculadoraTarifas calculadora;
        boolean esTemporadaAlta=false;
        for (String mes:mesesTemporadaAlta) {
        	if (fecha.toLowerCase().contains(mes)) {
        ...
        for (int i = 0; i < cantidad; i++) {
            int costoTiquete = calculadora.calcularTarifa(vuelo, cliente);
            Tiquete tiquete = GeneradorTiquetes.generarTiquete(vuelo, cliente,costoTiquete);
            cliente.agregarTiquete(tiquete); 
            costoTotal += costoTiquete;
        }
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
    	...
    	int saldoPendiente = cliente.calcularValorTotalTiquetesSinUsar();
